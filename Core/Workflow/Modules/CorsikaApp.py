""" The CorsikaApp class provides a simple way for CTA users to
  execute a Corsika Application.
"""
__RCSID__ = "$Id$"
...
class CorsikaApp:
  """
    The Class containing the CorsikaApp Module code
    It requires the following Input Parameters to be defined:
     ...
     Corsika Executable:  Executable to be passed to the application
  """
  ...
  def __init__( self ):
    ...
    """
    self.log = DIRAC.gLogger.getSubLogger( "CorsikaApp" )
    self.csArguments = []
  ...
  def execute( self ):
    ...
    if not ret['OK']:
      self.log.error( 'Failed to execute corsika:', ret['Message'] )
      return DIRAC.S_ERROR( 'Can not execute corsika' )
