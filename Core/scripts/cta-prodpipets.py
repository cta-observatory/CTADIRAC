...
def setRunNumber( optionValue ):
  global run_number
  run_number = '%06d' % int(optionValue)
...
def setProdName( optionValue ):
  global prodName
  prodName = optionValue
  return DIRAC.S_OK()

def setPathRoot( optionValue ):
  global pathroot
  pathroot = optionValue
  return DIRAC.S_OK()

def setNbShowers( optionValue ):
  global nbShowers
  nbShowers = str(int(optionValue))
  return DIRAC.S_OK()
...
def main():
  ...
  Script.registerSwitch( "P:", "prodName=", "ProdName", setProdName )
  Script.registerSwitch( "R:", "pathroot=", "PathRoot", setPathRoot )
  Script.registerSwitch( "N:", "nbShowers=", "NbShowers", setNbShowers )
 ...
 ###########
  ...
  createGlobalsFromConfigFiles(prodName,pathroot,corsikaTemplate,version)
...
#### create a file to DISABLE_WATCHDOG_CPU_WALLCLOCK_CHECK ################
  ...
  if savecorsika == 'True':
    ...
    ret = dirac.addFile(corsikaOutFileLFN, corsikaFileName, storage_element)  # obsolete
  ...
  # put and register corsikaTarFile:
    ...
    ret = dirac.addFile(corsikaTarFileLFN, corsikaTarName, storage_element) # obsolete
...
def createGlobalsFromConfigFiles(prodName,pathroot,corsikaConfigFileName,version):
    ...
    if (len(line.split())>0):
      for word in line.split():
        if line.split()[0] is not '*' and word in keywordsList:
          ...
          if word == 'NSHOW': 
            matchfound = 'True'
            dict[word] = [str(nbShowers)]
          else:
            dict[word] = value
      if matchfound == 'True':
        tempFile.write( line.replace(value[0],str(nbShowers) ) )
        matchfound = 'False'
      else:
        tempFile.write( line )
  tempFile.close
